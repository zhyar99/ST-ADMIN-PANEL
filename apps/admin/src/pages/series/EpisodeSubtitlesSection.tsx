...
/**
 * Subtitle tracks for one episode. The episode-side counterpart of the movies
 * page's SubtitlesSection, with the same one-track-per-language rule.
 */
export default function EpisodeSubtitlesSection({ episode }: { episode: EpisodeRef }) {
  const queryClient = useQueryClient();
  const { isAdmin } = useAuth();
  const canManage = isAdmin();

  const queryKey = ['admin', 'episodes', episode.episodeId, 'subtitles'] as const;
  ...
  const subtitlesQuery = useQuery({ queryKey, queryFn: () => listEpisodeSubtitles(episode) });
  ...
  const addMutation = useMutation({
    mutationFn: () =>
      addEpisodeSubtitle(episode, {
        language,
        // Exactly one of these is sent — the server rejects both or neither.
        ...(mode === 'library' ? { asset_id: asset?.id } : { external_url: externalUrl.trim() }),
      }),
    onSuccess: () => {
      setAsset(null);
      setExternalUrl('');
  ...
  const deleteMutation = useMutation({
    mutationFn: (subtitleId: string) => deleteEpisodeSubtitle(episode, subtitleId),
    onSuccess: () => {
      ...
      {canManage && (
        ...
        >
          ...
          <div className="flex flex-wrap items-end gap-3">
            <div>
              <label
                htmlFor="episode-subtitle-language"
                className="mb-1 block text-xs font-medium text-slate-400"
              >
                Language
              </label>
              <select
                id="episode-subtitle-language"
                value={language}
